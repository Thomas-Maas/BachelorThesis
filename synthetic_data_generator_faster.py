...
def insert_person(entry_count: int, columns: list, table_name: str):
    ...
    for row in entry:
        ...
        values_sql.append(f"(%s, %s, {', '.join(['%s'] * len(columns))}, Bdd(%s))")
        params.extend([id, name, *row, sentence])
    ...
    print(f"Took {time.time() - org_time:.2f} seconds to insert {len(entry)} entries for {name} with id {id}")
    org_time = time.time()
    division_cache = {
    ...
    # --- _dualrelation insert ---
    values_sql = []
    params = []
    for key, division in division_cache.items():
        for i, prob in enumerate(division):
            values_sql.append("(%s, %s)")
            params.extend([f"{key}={i}", prob])

    dual_query = f"""
    INSERT INTO {table_name}_dualrelation (varval, prob)
    VALUES {', '.join(values_sql)}
    """
    cur.execute(dual_query, params)

    # --- _triplerelation insert ---
    values_sql = []
    params = []
    for key, division in division_cache.items():
        for i, prob in enumerate(division):
            values_sql.append("(%s, %s, %s)")
            params.extend([key, i, prob])

    triple_query = f"""
    INSERT INTO {table_name}_triplerelation (var, val, prob)
    VALUES {', '.join(values_sql)}
    """
    cur.execute(triple_query, params)
    print(f"Took {time.time() - org_time:.2f} seconds to insert dual and triple relations for {name} with id {id}")
...
def add_persons(table_name, amount_people, entries, columns=list):
    ...
    for x in range(amount_people):
        insert_person(tables_each_person[x], columns, table_name)
