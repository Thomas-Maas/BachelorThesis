...
def insert_person(entry_count: int, columns: list, table_name: str):
    """
    ...
    for _ in range(entry_count):
        generated_entry = []
        for y in columns:
            generated_entry.append(get_random_bogus())
        if generated_entry not in entry:
            entry.append(generated_entry)
        #print(f"Generated entry: {generated_entry}")
    bdd_makers[id] = Bdd_sentence_maker(id, columns)
    entry = list(set(tuple(row) for row in entry)) # Remove duplicates by converting to a set and back to a list
    print(f"Generated {len(entry)} unique entries for {name} in {time.time() - temp_time:.2f} seconds")
    temp_time = time.time()
    sentence_occurrences = dict()
    for row in entry:
        counter = 0
        for value in row:
            bdd_makers[id].input_value(value, columns[counter])
            counter += 1
    ...
    for row in entry:
        ...
        #print(f"Data dict for row: {data_dict}")
        sentence = bdd_makers[id].get_total_sentence(**data_dict)
        sentence_occurrences = bdd_makers[id].get_variable_counts()
        #print(f"Sentence: {sentence}, Occurrences: {sentence_occurrences}")
        #print(f"Person: {name}, Sentence: {sentence}, Occurrences: {sentence_occurrences}")
        #print(f"Inserting person: {name} with data {data_dict} and sentence '{sentence}' into table '{table_name}'")
        cur.execute(
            f"""
            INSERT INTO {table_name}
            (id, name, {', '.join(columns)}, _sentence)
            VALUES (%s, %s, {', '.join(['%s'] * len(columns))}, Bdd('{sentence}'))
            """,
            (id, name, *row)
        )
    print(f"Inserted {len(entry)} entries into {table_name} for {name} in {time.time() - temp_time:.2f} seconds")
    temp_time = time.time()
    # Generate and cache all divisions
    division_cache = {
        key: generate_proportions(count)
        for key, count in sentence_occurrences.items()
    }

    # Update _dict table with probability string
    for key, division in division_cache.items():
        sentence_str = ''.join(f"{key}={i}:{division[i]};" for i in range(len(division)))
        cur.execute(
            f"UPDATE _dict SET dict = add(dict, %s) WHERE name = '{table_name}_dict'",
            (sentence_str,)
        )

    # Update dualrelation table
    for key, division in division_cache.items():
        for i, prob in enumerate(division):
            cur.execute(
                f"INSERT INTO {table_name}_dualrelation (varval, prob) VALUES (%s, %s)",
                (f"{key}={i}", prob)
            )

    # Update triplerelation table
    for key, division in division_cache.items():
        for i, prob in enumerate(division):
            cur.execute(
                f"INSERT INTO {table_name}_triplerelation (var, val, prob) VALUES (%s, %s, %s)",
                (key, i, prob)
            )
    print(f"Inserted {len(entry)} entries into dicts for {name} in {table_name} in {time.time() - temp_time:.2f} seconds")


def add_persons(table_name, amount_people, entries, columns=list):
    """
    Add a number of persons (witnesses or suspects) with random attributes to the respective table.
    """
    for _ in range(amount_people):
        insert_person(entries, columns, table_name)
...
def create_table(table_name, columns, persons, rows, force_total_rows=True):
    ...
    #Check if rows divisible by persons
    if force_total_rows:
        if rows % persons != 0:
            entries = int(round(rows / persons))
        else:
            entries = int(rows / persons)
        add_persons(table_name, persons, entries, columns)
    else:
        entries = rows
        add_persons(table_name, persons, entries, columns)
